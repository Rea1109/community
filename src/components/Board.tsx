...
import { checkLink, replaceText, replaceTitle } from '../commons/utility';
    ...
    return page === 'list' ? (
        ...
        <>
            ...
            <BoardContent ml="26px" mr="26px" size="1.6rem">
                {checkLink(content || '').map((el: any, index: number) =>
                    el.includes('https') ? (
                        <BoardContentLink key={index} onClick={onClickLink(el)}>
                            {el}
                        </BoardContentLink>
                    ) : (
            ...
            {imageUrl?.map((url, index) => (
                <BoardImage key={index} src={url} alt="content" height="auto" />
            ))}
        </>
    );
}
